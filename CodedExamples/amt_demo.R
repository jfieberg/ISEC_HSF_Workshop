...
# - Basics of 'amt'
# - HSFs in 'amt'
#   - Processing data & attaching covariates
#   - Fitting a model
...
# Recall that HSFs assume independent points. Our "GPS" data were simulated
# from a step-selection processes, so they are not independent. As with
# real data, we can subsample our data for "biological independence".

# Let's keep 2 locations per day. I will choose the noon and midnight location 
# each day.
sub <- gps %>% 
  filter(hour(t) %in% c(0, 12))

# Now we can format 'sub' as a track_xyt to be ready to work with it in 'amt'
trk <- sub %>% 
...
# To fit an HSF, we now need to sample the available locations. We also need to
# define the scale at which we sample available points. We can do this with
# the function amt::random_points().
?random_points

# The default method for an object of class "track_xy" fits a 100% MCP
# around the used locations and randomly sample available points within that
# polygon. Note the default arguments:
#   - level = 1 (100% isopleth)
#   - hr = "mcp" (MCP home range)
#   - type = "random" (random sampling; other options in ?sp::spsample)
# This corresponds to within-home-range (third order) habitat  selection. 

# Note the other default is 10x as many available points as used points. 
#   - n = nrow(x) * 10
# Let's see how to increase that to 100x. We'll keep all other defaults.
set.seed(20220626)
rand3 <- trk %>% 
  random_points(n = nrow(.) * 100)

# Note that this object is now of class "random_points"
class(rand3)

# Also note that "random_points" has a 'plot()' method
plot(rand3)

# What if we wanted to use the entire extent of our raster?
# (Somewhat closer to second-order habitat selection)
rand_rast <- hab %>% 
  st_bbox() %>% 
  st_as_sfc() %>% 
  st_as_sf() %>% 
  # Now this is class "sf", so see 'random_points.sf()'
  random_points(n = nrow(trk) * 100,
                # adds observed points
                presence = trk)

plot(rand_rast)

# Let's proceed with the 'rand3' object.

# Now we need to attach our environmental covariate data to our observed and
# available points. We can do this with 'extract_covariates()'
?extract_covariates

# By using a RasterStack, we will extract the values for all layers in
# the stack.
hsf_dat <- rand3 %>% 
  extract_covariates(hab) %>% 
  # Format "cover" with correct categories
  mutate(cover = factor(cover, labels = c("grassland", "forest", "wetland")))

head(hsf_dat)

# ... fitting a model ----
# We are almost ready to fit a model. Remember, the equivalence to the IPP
# holds if we assign large ("infinite") weights to the available points
hsf_dat <- hsf_dat %>% 
  mutate(weight = case_when(
    case_ ~ 1, # case_ == TRUE --> assign 1
    !case_ ~ 5000 # case_ == FALSE --> assign 5000
  ))

# We can now fit the model using base R's 'glm()'
hsf <- glm(case_ ~ forage + temp + I(temp^2) + pred + cover,
           family = binomial(), data = hsf_dat, weights = weight)

# Check the summary
summary(hsf)

# ... model interpretation ----
# How many times more locations do we expect in a habitat with predator
# density of 5 predators/100 km2 vs 10 predators/100 km2?

x1 <- data.frame(forage = mean(hab$forage[]),
                 temp = mean(hab$temp[]),
                 pred = 5,
                 cover = factor("grassland", levels = levels(hsf_dat$cover)))
x2 <- data.frame(forage = mean(hab$forage[]),
                 temp = mean(hab$temp[]),
                 pred = 10,
                 cover = factor("grassland", levels = levels(hsf_dat$cover)))

# Calculate log-RSS
lr1 <- log_rss(hsf, x1 = x1, x2 = x2, ci = "se")
# Exponentiate to get RSS
exp(lr1$df[, c("log_rss", "lwr", "upr")])

# We expect ~ 5.9x as many locations in x1 as x2 (95% CI 3.5 -- 9.8)
...
# How does RSS change with temp?
# Compare RSS for range of temp with mean temp.
x1 <- data.frame(forage = mean(hab$forage[]),
                 temp = seq(10, 25, length.out = 50),
                 pred = mean(hab$pred[]),
                 cover = factor("grassland", levels = levels(hsf_dat$cover)))
...
lr2$df %>% 
  mutate(rss = exp(log_rss),
         rss_lwr = exp(lwr),
         rss_upr = exp(upr)) %>% 
  ggplot(aes(x = temp_x1, y = rss, ymin = rss_lwr, ymax = rss_upr)) +
  geom_ribbon(color = "black", fill = "gray80", linetype = "dashed") +
  geom_line(size = 1) +
  geom_hline(yintercept = 1, color = "red", linetype = "dashed") +
  xlab("Mean Annual Temperature (°C)") +
  ylab("RSS vs. Mean Temp") +
  theme_bw()

# ... mapping ----
# We can map the exponential habitat selection function.
# This can also be interpreted as the RSS vs a habitat where all covariates
# have the value 0.
...
map_dat <- as.data.frame(hab, xy = TRUE) %>% 
  ...
  # Calculate linear predictor (without intercept)
  mutate(lp = b[["forage"]] * forage +
           b[["temp"]] * temp +
           b[["I(temp^2)"]] * temp^2 +
           b[["pred"]] * pred +
           b[["coverforest"]] * (cover == "forest") +
           b[["coverwetland"]] * (cover == "wetland")) %>% 
  # Exponentiate
  mutate(hsf = exp(lp)) %>% 
  # Normalize (sum to 1 across raster)
  mutate(hsf_norm = hsf/sum(hsf))

# Map eHSF
ggplot(map_dat, aes(x = x, y = y)) +
  geom_raster(aes(fill = hsf)) +
  coord_sf() +
  xlab(NULL) +
  ylab(NULL) +
  scale_fill_viridis_c(name = "eHSF") +
  theme_bw()

# Map Pr(Use) [eHSF normalized] (lots of assumptions here!)
ggplot(map_dat, aes(x = x, y = y)) +
  geom_raster(aes(fill = hsf_norm)) +
  coord_sf() +
  xlab(NULL) +
  ylab(NULL) +
  scale_fill_viridis_c(name = "Pr(Use)") +
  theme_bw()

# We can also calculate (log) RSS vs. the mean conditions
x1 <- as.data.frame(hab, xy = TRUE) %>% 
  mutate(cover = factor(cover_VALUE, levels = levels(hsf_dat$cover)))
...
lr3 <- log_rss(hsf, x1, x2)

# Map log-RSS vs. mean habitat
lr3$df %>% 
  ...
  ggtitle("log-RSS vs. Mean Habitat") +
  theme_bw()
...
(upd_sl <- update_sl_distr(m1, beta_log_sl = "log(sl_)"))
...
# Notice our estimated kappa is negative. The von Mises is not defined for
...
# around +/- pi. We can simply multiply the estimated kappa by -1 and change
# mu to pi.
...
# We can use 'amt::log_rss()' just as we did for the HSF above.
